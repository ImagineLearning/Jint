            ...
            if (valueType == typeof(Func<JsValue, JsValue[], JsValue>))
            {
                ...
                if (type.IsGenericType)
                {
					...
					else if (type.IsSubclassOf(typeof(System.MulticastDelegate)))
					{
						...
						var parameters = new ParameterExpression[arguments.Count()];
						for (var i = 0; i < parameters.Count(); i++)
						{
							parameters[i] = Expression.Parameter(typeof(object), arguments[i].Name);
						}
						var vars = Expression.NewArrayInit(typeof(JsValue), parameters.Select(p => Expression.Call(null, typeof(JsValue).GetMethod("FromObject"), Expression.Constant(_engine, typeof(Engine)), p)).ToArray());
						...
						var lambdaExpression = Expression.Lambda(callExpression, parameters);
						var dynamicExpression = Expression.Invoke(lambdaExpression, parameters);

						return Expression.Lambda(type, dynamicExpression, new ReadOnlyCollection<ParameterExpression>(parameters));
					}
				}
                else
                {
